...
function MtGoxClient(key, secret) {
	...
	self.quote = function(type, amount, callback) {
		self.makePublicRequest("BTCUSD/money/order/quote", {"type": type, "amount": amount}, callback);
	};

	// price is an optional argument, if not used it must be set to null
	self.add = function(type, amount, price, callback) {
		if (price) { var args = {"type": type, "amount": amount, "price": price}; }
		else { var args = {"type": type, "amount": amount}; }
		self.makeRequest("BTCUSD/money/order/add", args, callback);
	};

	self.cancel = function(id, callback) {
		self.makeRequest("BTCUSD/money/order/cancel", {"oid": id}, callback);
	};

	// not currently implemented
	self.result = function(type, order, callback) {
		self.makeRequest("BTCUSD/money/order/result", {"type": type, "order": order}, callback);
	};
	...
	self.history = function(currency, page, callback) {
		if (page) { var args = {"currency": currency, "page": page}; }
		else { var args =  {"currency": currency}; }
